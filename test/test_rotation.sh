...
echo -e "Command: ssh -i $KEY_PATH ubuntu@$PUBLIC_IP \"bash ssh_keys_rotation.sh $PRIVATE_IP\"\n\n"

ssh -i $KEY_PATH ubuntu@$PUBLIC_IP "bash ssh_keys_rotation.sh $PRIVATE_IP"
